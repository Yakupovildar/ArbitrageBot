...
from sector_ui import sector_ui
...
class SimpleTelegramBot:
    ...
    async def handle_command(self, chat_id: int, command: str, user_id: int):
        ...
        if command == "/start":
            ...
            await subscription_manager.activate_trial_period(user_id)
...
👋 Дружище, пора закрывать позицию по GAZP/GAZPF!
...
🎯 **Важно:** Используйте USERNAME без @, например: ildaryakupovv
...
🎯 *Быстрое управление ботом:*"""
        ...
        # ===== НОВЫЙ СЕКТОРНЫЙ ИНТЕРФЕЙС =====
            ...
            # Показываем секторный интерфейс выбора пар (БЕЗ рекомендованных)
            keyboard = self.user_settings.get_instruments_keyboard(user_id, self.config.MONITORED_INSTRUMENTS)
            instruments_text = f"""📈 **ВЫБОР ТОРГОВЫХ ПАР ПО СЕКТОРАМ**

🎯 **Всего доступно: {len(self.config.MONITORED_INSTRUMENTS)} торговых пар**

📊 **Организация по экономическим секторам:**
• 🔵 Голубые фишки - крупнейшие компании
• 🏦 Банки - финансовый сектор  
• ⛽ Нефть и газ - энергетические компании
• 🏭 Металлургия - металлургические компании
...
• Только выбранные пары будут мониториться"""
            ...
            # Возврат к главному меню секторов
            message = sector_ui.get_sectors_summary_message()
            keyboard = sector_ui.get_sectors_menu_keyboard()
            await self.edit_message_text(chat_id, callback_query["message"]["message_id"], message, keyboard)
            await self.answer_callback_query(callback_query_id, "Секторы")
            
        elif callback_data.startswith("sector_") and not callback_data.startswith("sector_select") and not callback_data.startswith("sector_deselect"):
            # Выбор конкретного сектора
            if callback_data == "sector_all_available":
                # Показываем все доступные пары
                message = "✅ **ВСЕ ДОСТУПНЫЕ ТОРГОВЫЕ ПАРЫ**\n\n"
                available_count = len(ui_restrictions.status_manager.active_pairs)
                message += f"Активных пар: {available_count}\n\n"
                
                # Показываем первые 20 пар
                for i, pair_key in enumerate(list(ui_restrictions.status_manager.active_pairs)[:20]):
                    message += f"{i+1}. {pair_key}\n"
                
                if available_count > 20:
                    message += f"... и еще {available_count - 20} пар"
                
                keyboard = {"inline_keyboard": [[{"text": "🔙 К секторам", "callback_data": "sectors_menu"}]]}
                
            elif callback_data == "sector_blocked":
                # Показываем заблокированные пары  
                message = "🚫 **ЗАБЛОКИРОВАННЫЕ ПАРЫ**\n\n"
                blocked_count = len(ui_restrictions.status_manager.blocked_pairs)
                message += f"Заблокированных пар: {blocked_count}\n\n"
                
                for i, pair_key in enumerate(list(ui_restrictions.status_manager.blocked_pairs)[:15]):
                    message += f"{i+1}. {pair_key}\n"
                
                if blocked_count > 15:
                    message += f"... и еще {blocked_count - 15} пар"
...
Вы получаете максимум возможностей бота!"""
                    ...
                    subscription_text = f"""🆓 **Пробный период активен**

📅 **Осталось дней:** {remaining_days}/7
🚀 **Сигналы:** Безлимитно до окончания
⚠️ **После окончания:** Потребуется подписка

💎 **Премиум подписка:**
• Безлимитные сигналы навсегда
...
💰 **Стоимость:** 10 USDT/месяц"""
                    ...
                    subscription_text = """⏰ **Пробный период истек**
...
⏰ Время: {datetime.now().strftime('%H:%M:%S %d.%m.%Y')}"""
    ...
    async def _test_monitoring_task(self, user_id: int):
        ...
        while self.test_monitoring_active.get(user_id, False):
            try:
                # ПРОВЕРЯЕМ СТАТУС БИРЖИ ПЕРЕД ОТПРАВКОЙ СИГНАЛОВ
                if not self.config.is_trading_hours():
                    market_status = self.config.get_trading_status_message()
                    logger.info(f"Тестовый мониторинг приостановлен: биржа закрыта")
                    await self.send_message(user_id, f"⏸️ **ТЕСТОВЫЙ МОНИТОРИНГ ПРИОСТАНОВЛЕН**\n\n{market_status}\n\n🔄 Возобновится автоматически когда биржа откроется")
    ...
    async def daily_validation_task(self):
        ...
        while True:
            try:
                ...
                # БЫСТРАЯ индивидуальная валидация
                logger.info("🚀 БЫСТРАЯ индивидуальная валидация торговых пар...")
                await ui_restrictions.status_manager.validate_all_pairs_fast()
