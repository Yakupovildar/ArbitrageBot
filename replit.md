...
- **Спред-расчеты исправлены**: Устранена ошибка с 10546039% спредами - теперь корректно 0.7-3%
- **Склонение лотов**: Исправлено "1 лотов" → "1 лот", корректная грамматика русского языка
- **Логика закрытия позиций**: Позиции закрываются только при снижении спреда на 50%+ И ниже 0.5%
- **Команда проверки**: /check_settings показывает все сохраненные настройки из PostgreSQL
