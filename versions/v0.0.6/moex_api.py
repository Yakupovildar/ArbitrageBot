...
class MOEXAPIClient:
    ...
    def _convert_futures_price_to_rubles(self, ticker: str, price: float) -> float:
        """Конвертация цены фьючерса в рубли используя точные коэффициенты из конфигурации"""
        
        # Используем точные коэффициенты из config.py
        multiplier = self.config.get_futures_lot_value(ticker)
        
        # ПРАВИЛЬНАЯ ЛОГИКА: умножаем цену на коэффициент для получения цены в рублях
        converted_price = price * multiplier
        
        logger.debug(f"Конверсия {ticker}: {price} × {multiplier} = {converted_price}₽")
