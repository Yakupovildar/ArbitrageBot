...
class MOEXAPIClient:
    ...
    def _convert_futures_price_to_rubles(self, ticker: str, price: float) -> float:
        ...
        special_conversions = {
            ...
            'SPZ5': 1000,    # SNGSP - делить на 1000 (спред 26.59%)
            'SZZ5': 1000,    # SGZH - делить на 1000 (спред 4.72%)
        }
    ...
    async def get_multiple_quotes(self, instruments: Dict[str, str]) -> Dict[str, Tuple[Optional[float], Optional[float]]]:
        """БЫСТРОЕ получение котировок для множества инструментов"""
        # Убираем принудительную очистку кеша для избежания блокировок
        logger.info("📊 Получение котировок для валидации (без очистки кеша)")
        ...
        # ОСТОРОЖНАЯ валидация: избегаем блокировок от API
        semaphore = asyncio.Semaphore(1)  # По 1 запросу для избежания rate limit
        
        async def fetch_pair(stock_ticker, stock_task, futures_task):
            async with semaphore:
                # Получение цен с минимальными задержками для соблюдения лимитов
                await asyncio.sleep(0.2)  # Минимальная задержка для rate limit
