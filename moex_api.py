...
class MOEXAPIClient:
    ...
    def _convert_futures_price_to_rubles(self, ticker: str, price: float) -> float:
        ...
        # Фьючерсы которые уже торгуются в рублях за акцию (правильный масштаб)
        if ticker in ['SBERF', 'GAZPF']:  # Сбер и Газпром - цена уже в рублях за акцию
            return price
        
        # Фьючерсы которые торгуются в пунктах - нужна точная конвертация
        elif ticker in ['LKZ5']:  # Лукойл: 64003 пунктов = 6336 рублей
            return price / 10.1
            
        elif ticker in ['GKZ5']:  # ГМК Норникель: 1369 пунктов = 128 рублей
            return price / 10.7
            
        elif ticker in ['VBZ5']:  # ВТБ: 8410 пунктов = 79 рублей
            return price / 106.5
            
        elif ticker in ['RNZ5']:  # Роснефть: 48638 пунктов = 457 рублей
            return price / 106.5
            
        elif ticker in ['TNZ5']:  # Татнефть: 1422 пунктов = 711 рублей
            return price / 2.0
            
        elif ticker in ['ALZ5']:  # АЛРОСА: 5031 пунктов = 48 рублей
            return price / 104.4
