...
class Config:
    ...
    def __post_init__(self):
        ...
        if not self.MONITORED_INSTRUMENTS:
            self.MONITORED_INSTRUMENTS = {
                # ===== РЕКОМЕНДОВАННЫЕ ПАРЫ (проверены 15.08.2025) =====
                "GAZP": "GZZ5",   # Газпром - декабрь 2025 ✅
                "SBER": "SRZ5",   # Сбербанк - декабрь 2025 ✅  
    ...
    def get_futures_lot_value(self, ticker: str) -> float:
        ...
        futures_multipliers = {
            ...
            # Рекомендованные пары - конверсия из копеек в рубли
            "GZZ5": 0.01,    # Газпром Z5 - копейки -> рубли
            "SRZ5": 0.01,    # Сбербанк Z5 - копейки -> рубли  
            "LKZ5": 0.01,    # Лукойл Z5 - копейки -> рубли
            "GKZ5": 0.01,    # ГМКН Z5 - копейки -> рубли
            "VBZ5": 0.01,    # ВТБ Z5 - копейки -> рубли
            "RNZ5": 0.01,    # Роснефть Z5 - копейки -> рубли
            "MGZ5": 0.01,    # Магнит Z5 - копейки -> рубли
            
            # Старые контракты (голубые фишки)
